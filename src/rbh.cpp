...
static std::vector<size_t> filter_contained_hsps(const std::vector<NormRec> &hsps){
    ...
    for(size_t i = 0; i < hsps.size(); i++){
        for(size_t j = 0; j < hsps.size(); j++){
            if(i == j) continue;
            if(hsps[i].dir != hsps[j].dir) continue;
...
static int recalc_coverage(const std::vector<NormRec> &hsps,
    ...
    if(ranges.empty() || tx_len <= 0) return NA_INTEGER;
...
void rbh_from_sorted_norm_cpp(std::string sorted_norm_fn,
    ...
    auto flush_pair = [&](){
        ...
        double q2s_qcovs = recalc_coverage(pair_hsps, keep_idx, 1);
        double s2q_qcovs = recalc_coverage(pair_hsps, keep_idx, 2);
        ...
        if(q2s_qcovs == q2s_qcovs) ci_q2s = pident_pair * q2s_qcovs * 1e-4;
        if(s2q_qcovs == s2q_qcovs) ci_s2q = pident_pair * s2q_qcovs * 1e-4;
