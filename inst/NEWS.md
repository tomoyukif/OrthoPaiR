...
- Replaced archive/backfill dependencies by moving required helpers into active scripts.
- Updated reorg step to consume pairwise TSVs under `orthopair/` and write graph/table outputs under `reorg_out/`.
- Unified internal/output naming from `query_/subject_` to `genome1_/genome2_`.
- Removed HDF5 dependency from current MSA/plot input paths and switched to `orthopair` or `reorg_out` directory inputs.
- Updated README and vignette to document the current stepwise workflow:
  `reformatFiles()` -> `rbh()` -> `orthopair()` -> `reorgOrthopairs()`.
