...
.getOrphan <- function(synog_gene, g2g_graph){
    ...
    out <- list(query = query_orphan, subject = subject_orphan)
}
...
.getGFFlist <- function(h5, mp = FALSE){
    ...
    if(mp){
        ...
        out <- list(query_gff = query_gff, subject_gff = subject_gff,
                    mp_query_gff = mp_query_gff, mp_subject_gff = mp_subject_gff)
...
#'     q_gff <- .importAllGFF(as.vector(h5$files$query_gff))
#'     s_gff <- .importAllGFF(as.vector(h5$files$subject_gff))
